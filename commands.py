...
def calendar(text):
    ...
    for ev in events:
        if datetime.datetime.now(tz=timezone("Europe/Amsterdam")) <= ev["start"] <= datetime.datetime.today(datetime.timezone(offset=datetime.timedelta(hours=1))) + relativedelta.relativedelta(weeks=2):
...
def get_num(text):
    while True:
        ...
        if res_full is not None and res_full is not []:
